...
def load_data(path):
    ...
    for country, g in df.groupby("country_code"):
        g = g.sort_values("year")  # ensure chronological order for time-series split
        X_full = g[FEATURES].reset_index(drop=True)  # exclude 'year' from predictors

        for tgt in FEATURES:
            X = X_full.drop(columns=[tgt])
            y = X_full[tgt].reset_index(drop=True)
            if len(y) < 10:        # sequence too short → skip
                continue
            # skip constant sequences that break metric calculations
...
def scorer(y_true, y_pred):
    ...
    # ❶ 把无穷大、NaN 换成一个“极差但有限”的分数，避免 GridSearchCV 抛弃该组合
    if any(np.isnan(v) or np.isinf(v) for v in m.values()):
        return -9.99    # 仍然很差，但可比较
    ...
    for algo in ["XGB", "RF", "ARIMA"]:
        ...
        for i, (country, tgt) in enumerate(keys, 1):
            ...
            if algo == "XGB":
                ...
                print("best_score = %.2lf"%(best_score))
            ...
            guiding_final = np.nan if any(np.isnan(v) or np.isinf(v) for v in m.values()) else get_scores.guiding_score(m)
            ...
            row = {
                ...
                "target": tgt,
                **m,
                "guiding_score": guiding_final,      # unified column name
                "best_guiding_cv": best_score,
