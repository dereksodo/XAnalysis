...
def main():
    ...
    df = pd.read_csv(os.path.join(Paths.FIGURE_DIR, "Chapter6", "ridge3_kfold_results.csv"))
    ...
    # Save to CSV
    country_feature_out_path = os.path.join(Paths.FIGURE_DIR, "Chapter6", "ridge3_country_feature_scores.csv")
    ...
    plt.tight_layout()
    out_path = os.path.join(Paths.FIGURE_DIR, "Chapter6", "guiding_score_ridge3.png")
    ...
    # We'll plot by feature (indicator), average value for Ridge3 and Naive
    ...
    for idx, (metric, title) in enumerate(zip(metric_keys, metric_titles)):
        ...
        ax.plot(features_metrics, ridge_vals, marker="o", label="Ridge3", color="skyblue", linestyle="-")
    ...
    fig2.tight_layout()
    out_path2 = os.path.join(Paths.FIGURE_DIR, "Chapter6", "ridge3_vs_naive_by_metric.png")
