...
def get_fold_years():
    ...
    return folds
def evaluate_lambda_pair(df_sub, country_code, feature_code, indicator_cols, cross_feature_indices, folds, lambda_A, lambda_M, return_result=False):
    """
    Evaluate a pair of (lambda_A, lambda_M) for a given country-feature and its folds.
    Returns guiding score (and optionally the best result DataFrame).
    """
...
def main():
    ...
    for country_code in country_codes:
        for feature_code in feature_cols:
            ...
            # Coordinate descent for lambda_A, lambda_M
            init_lambda_A = 1.0
            init_lambda_M = 1.0
            candidate_lambdas = [0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0]
            best_score = -np.inf
            best_lambda_A = init_lambda_A
            best_lambda_M = init_lambda_M
            improved = True
            while improved:
                improved = False
                # Fix lambda_M and optimize lambda_A
                for lambda_A in candidate_lambdas:
                    score = evaluate_lambda_pair(
                        df_sub, country_code, feature_code, indicator_cols, cross_feature_indices, folds,
                        lambda_A, best_lambda_M, return_result=False)
                    if score > best_score:
                        best_score = score
                        best_lambda_A = lambda_A
                        improved = True
                # Fix lambda_A and optimize lambda_M
                for lambda_M in candidate_lambdas:
                    score = evaluate_lambda_pair(
                        df_sub, country_code, feature_code, indicator_cols, cross_feature_indices, folds,
                        best_lambda_A, lambda_M, return_result=False)
                    if score > best_score:
                        best_score = score
                        best_lambda_M = lambda_M
                        improved = True
            # Evaluate final result
            final_score, best_result = evaluate_lambda_pair(
                df_sub, country_code, feature_code, indicator_cols, cross_feature_indices, folds,
                best_lambda_A, best_lambda_M, return_result=True)
    ...
    if results_rows:
        ...
        output_path = os.path.join(Paths.FIGURE_DIR,"Chapter6", "ridge3_kfold_results.csv")
        final_df.to_csv(output_path, index=False)
        print(f"Saved full results to {output_path}")
        param_path = os.path.join(Paths.FIGURE_DIR,"Chapter6", "ridge3_kfold_best_params.csv")
